...
def index1(request):
    request.session['login_user_id'] = 'ggoreb'
    request.session['login_user_address'] = 'busan'
...
def download(request):
    with open('board.zip', 'rb') as file:
        res = HttpResponse(file, content_type='application/octet-stream')
        res['Content-Disposition'] = 'attachment; filename=%s' % 'board.zip'
